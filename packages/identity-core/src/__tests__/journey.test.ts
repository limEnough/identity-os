...
describe('앞 축의 결과가 뒤 축의 출발점이 된다', () => {
  it('같은 발자국이라도 앞 축이 다르면 다른 결과에 닿을 수 있다', () => {
    ...
    function identityWalks(): number[][] {
      return [
        [0, 0, 0, 0, 0, 0, 0], // 「단단함」 — 윤곽을 세우는 쪽
        [1, 0, 0, 0, 0, 0, 0], // 「다정함」 — 모서리를 둥글리는 쪽
        [5, 3, 0, 0, 0, 0, 0], // 「호기심」 — 움직이는 쪽
        [3, 1, 0, 0, 0, 0, 0], // 「회복력」 — 오래 쌓이는 쪽
      ];
    }
  });
  ...
  it('임시 확정(유보)은 절반만 각인된다', () => {
    const decided = replayAxis(AXES[0], [0, 0, 0, 0, 0, 0, 0]);
    const tentative = replayAxis(AXES[0], [0, 0, 0, 0, 0, 3, 0]);
...
describe('주소', () => {
  it('걸어온 발자국을 모두 실어 보낸다', () => {
    ...
    expect(query).toBe('i=0.0.0.0.0.0.0&m=0.0.0.0.0.0.0&c=0.0.0.0.0.0.0');
