 ...
 * `median(acc)/n`을 재고 **여덟 축 지점**(봉인되는 자리이자 이름이 불리는 자리)에
 * 맞췄다. 한때 여섯~여덟 축의 평균을 썼는데, 걸을수록 쏠림이 커지는 탓에 여덟 축에서
 * 글자가 한쪽으로 5%p씩 기울었다 — 기준은 이름이 불리는 지점에 두는 게 맞다.
 *
 * 축 데이터를 크게 손보면 조용히 어긋난다. 그 관문이 code.test.ts의 분포 테스트다
 * (글자마다 38~62%, 한 자리가 5분의 1을 넘지 않기, 열여섯 자리 모두 도달).
 */
