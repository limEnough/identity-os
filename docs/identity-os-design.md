...
> **알려진 한계** — 글자마다는 거의 공평한 동전이 됐지만(47~52%), 네 갈래가 서로 **상관**돼 있다: 축 콘텐츠가 생동감·또렷함·밖으로를 함께 묶어 적어둔 탓에 대각선 모서리(`VKNO`·`QSRI`)에 무게가 몰린다(각 20%대). 제대로 풀려면 여덟 축 64개 결과의 각인을 다시 배분해야 하고, 그건 콘텐츠 작업이다. 열여섯 전부 도달 가능하고 한 자리가 30%를 넘지 않으므로 지금은 받아들이되, 축 콘텐츠 심화(§8 Phase 6) 때 함께 손본다.

### 4.6 결 서재 — 「한 구절, 그리고 그것이 온 곳」

끝에 한 번 주고 마는 선물은 삼십 초 만에 소비되고 끝난다. 그래서 **서재**로 만든다: 하루에 한 꾸러미씩, 반년이면 스무 칸. 그 자체가 나를 설명하는 물건이 되도록.

꾸러미 하나 = **구절 하나 + 그것이 온 책** + **노래 하나 + 왜 이 결에 놓였는지 우리가 적은 한 줄**.
...
- **하루에 하나.** 한 번에 다 꺼내 가면 서재가 아니라 목록이 된다. 다만 감춰두고 조르지는 않는다 — 이미 받았으면 받았다고 적고 내일을 알린다.
- 고르는 방식은 축과 같다(가까운 결, 이미 가져간 것은 제외). **넓히는 일은 화면 작업이 아니라 `shelf/passages.ts`·`tracks.ts`에 줄을 더하는 데이터 작업이다.** 줄을 더할 때는 출처를 반드시 확인할 것.
...
- 서재와 연표는 **판이 바뀌어도 지워지지 않는다.** 발자국(진행 중인 판)과 간직되는 것(누적)의 수명이 다르다.
...
3. **Phase 3 — 가이드북 v1 (완료)**: Identity 섹션이 채워진 My Life Guide 웹 뷰. Style 섹션은 잠금 상태로 노출(불변식의 시각화). PDF·Taste 편입은 미착수.
   ...
   3.6. **Phase 3.6 — 네 글자·서재·연표 (완료)**: 여정의 보상과 재방문의 축을 세웠다(§4.5~4.8). `Canon`을 네 글자로 접어 축마다 한 자리씩 채우고(`axis/code.ts`), 하루 한 꾸러미씩 쌓이는 결 서재(`shelf/`)와 판마다 날짜가 붙는 결 연표(`chronicle.ts`), 점수 없이 질문 하나를 내미는 겹쳐보기(`together.ts`)를 붙였다. 불변식 #4는 네 조건을 달아 개정했다(§1.2).

   **여전히 완전 정적이다.** 코드 산출·서재·지도·겹쳐보기(URL 합성)·연표(브라우저 기억) 전부 서버 없이 돈다 — 서버가 필요한 것은 OG 공유 카드 하나뿐이고, 그건 아직 하지 않았다.

4. **Phase 4 — 계정·실천 루프**: Supabase 도입, 실천 카드·회고·버전. 리마인드(주간 회고 프롬프트).
5. **Phase 5 — LLM 코치**: /api/coach 심화 모드 + 위기 필터 + Insight 추출.
6. **Phase 6 — 축 콘텐츠 심화 (Phase 3.5에서 개통 완료)**: 여덟 축이 모두 열렸으므로 남은 일은 확장이 아니라 심화다 — 입구·결과 변주 추가, 축마다의 표현 언어 다듬기, 질문 문구 A/B. 축마다 같은 엔진을 재사용하므로 전부 데이터 작업이다. **여기서 각인의 상관도 함께 푼다**(§4.5의 알려진 한계) — 64개 결과의 각인을 다시 배분하면 열여섯 자리가 더 고르게 갈린다. 서재의 구절·노래를 늘리는 일도 같은 성격의 데이터 작업이다.
...
- **서재 콘텐츠의 출처 오류**: 인용은 짧게, 출처를 반드시 확인하고, 가사·본문은 싣지 않는다(제목만 복사). 줄을 더하는 사람이 지켜야 할 규칙이 `shelf/passages.ts` 머리에 적혀 있다.
