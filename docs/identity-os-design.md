...
> **알려진 한계** — 글자마다는 거의 공평한 동전이 됐지만(47~52%), 네 갈래가 서로 **상관**돼 있다: 축 콘텐츠가 뻗어감·윤곽·밖으로를 함께 묶어 적어둔 탓에 대각선 모서리(`VKNO`·`QSRI`)에 무게가 몰린다(각 20%대). 열여섯 전부 도달 가능하고 한 자리가 30%를 넘지 않으므로 지금은 받아들인다.
>
> 원인은 배선에 있다. 여덟 축의 `projection`이 **전부 `sign: +1`이고 같은 순서로 같은 갈래를 읽는다** — 여섯 축이 `reach·form·outward`를, 두 축이 `reach·form·modern`을. 축마다 다른 세 극이라고 적혀 있어도 실제로는 **같은 3차원의 재명명**인 셈이다. `modern`이 여덟 축에서 값을 받으면서 두 축에서만 읽히는 것도 같은 자리의 문제다.
>
> 푸는 데는 셋이 함께 필요하다: (a) `projection` 재배치(`sign: -1`을 실제로 쓰기), (b) 64개 결과의 각인 재배분, (c) `CENTER` 재측정. 여기에 구절 29개·노래 30개의 결과 네 글자 이름 16개도 따라온다. 각인 재배분이 콘텐츠 작업이므로 **축 콘텐츠 심화(§8 Phase 6) 때 한꺼번에** 한다 — 따로 하면 같은 파일을 두 번 헤집게 된다.
...
6. **Phase 6 — 축 콘텐츠 심화 (Phase 3.5에서 개통 완료)**: 여덟 축이 모두 열렸으므로 남은 일은 확장이 아니라 심화다 — 입구·결과 변주 추가, 축마다의 표현 언어 다듬기, 질문 문구 A/B. 축마다 같은 엔진을 재사용하므로 전부 데이터 작업이다. **여기서 각인의 상관도 함께 푼다**(§4.5의 알려진 한계) — 64개 결과의 각인을 다시 배분하면 열여섯 자리가 더 고르게 갈린다. 맺음의 구절·노래를 늘리는 일도 같은 성격의 데이터 작업이다.
